...
function mountPlayerOverlay() {
  if (document.querySelector('.bili-strip')) return; // 已挂载则跳过

  enableScrollLock();

  // ====== Styles ======
  const style = document.createElement('style');
  style.id = 'bili-overlay-style';
  style.textContent = `
    .bili-strip {
      position: fixed;
      z-index: 999999;
  ...
  // 保存 MutationObserver 引用，供卸载时使用
  window.__biliOverlayMO = mo;
}
...
function unmountPlayerOverlay() {
  ...
  // 移除所有遮罩 DOM 元素
  document.querySelectorAll('.bili-strip, .bili-corner, .bili-focus-ring').forEach(el => el.remove());
  const style = document.getElementById('bili-overlay-style');
  if (style) style.remove();

  // 断开 MutationObserver
  if (window.__biliOverlayMO) {
    window.__biliOverlayMO.disconnect();
    window.__biliOverlayMO = null;
  }
}

// 初始化
chrome.storage.sync.get('playerMaskEnabled', (data) => {
  if (data.playerMaskEnabled === false) return;
  mountPlayerOverlay();
});

// 实时响应开关变化，无需刷新页面
chrome.storage.onChanged.addListener((changes) => {
  if (!('playerMaskEnabled' in changes)) return;
  if (changes.playerMaskEnabled.newValue === false) {
